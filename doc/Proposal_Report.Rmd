...
2.  For Subway USA, the corresponding dataset has over 1000 features combined with similar information as mentioned above for Smoothie King, for approximately 14,000 stores across the US. The main goal is to cluster the stores into different clusters where stores share similar features within the same cluster. We propose starting with a DBSCAN model as a baseline and evaluate the clusters empirically. Afterwards, we will perform a Hierarchical clustering and apply PCA for dimensionality reduction and compare the results.
